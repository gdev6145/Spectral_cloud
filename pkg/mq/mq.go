...
// Count returns the number of pending messages in topic for tenant.
...
// Topics returns summary information for all topics belonging to tenant.
func (q *Queue) Topics(tenant string) []TopicInfo {
	q.mu.Lock()
	defer q.mu.Unlock()
	tt := q.tenantTopics(tenant)
	out := make([]TopicInfo, 0, len(tt))
	for topic, msgs := range tt {
		out = append(out, TopicInfo{Topic: topic, Tenant: tenant, Pending: len(msgs)})
	}
	return out
}

// Purge removes all messages from topic for tenant. Returns the number removed.
